...
zu konstruieren. Diese liefern uns schlußendlich bilholomorphe
...
\begin{proof}
  ...
  \item Es gilt $f^{-1}\d[f] \in \Omega(X)$, und wegen $Rh^1_\hol(X) = 0$
    ...
    \end{align*}
    Also ist $fe^{-g} = $ const und aus $f(a) e^{-g(a)} = 1$
    erhalten wir
    \[
    f = e^g.
    \]
...
\begin{proof}
  ...
  \item $f(Y) \subset B$: Sei $y \in Y$ beliebig. Dann gilt
    ...
    erhalten wir aus dem Maximumprinzip
    \[
  ...
  \item $f$ ist biholomorph: $f$ nimmt jeden Wert gleichhäufig an, da
    ...
    holomorph. Das genügt nach \break\cite[Kor. 2.5]{For} bereits, um zu zeigen, dass $f$ biholomorph
    ist.
...
\begin{proof}
  ...
  Montel (vgl. \break\cite[Satz 4.9]{Frei}) erhalten wir die Kompaktheit von $M$.
\end{proof}
...
\begin{proof}
  ...
  dass $z \in h_n(B)$ und $w \in h_{\tilde n} (B)$ ist. Weiterhin ist
...
\begin{proof}
  ...
  $g \in \hol(Y)$ mit $g^2 = \phi|_Y$. Also ist \break$g(Y) \subseteq
...
\begin{proof}
  ...
  Kurve. Demnach besitzt $\omega$ nach \break\cite[Satz 10.15]{For} eine Stammfunktion und
...
\begin{thm}[Der Riemannsche Abbildungssatz]
  \label{thm:rmt}
  Sei $X$ eine Riemannsche Fläche mit holomorpher deRham-Gruppe $Rh_\hol^1(X) = 0$. Dann ist $X$
  konform äquivalent
  \begin{enumerate}
...
\begin{proof}
  ...
  \[
  H^1(X, \hol) \cong H^1(X, \hol)^\ast \cong H^0(X, \Omega) \cong
  \Omega(X) = 0.
  \]
  ...
  \[
  \phi: B \ra B, \quad\phi(z) := \frac{z- \tilde f_n(a)}{1 - z \overline{\tilde
      f_n(a)}},
  \]
  so erhalten wir zunächst $\bar f_n := \phi \circ \tilde
  ...
  Weiterhin gilt $r_n \leq r_{n+1}$ für jedes $n \in \N$, aus dem
  einfachen Grund, dass $h:
  f_{n+1} \circ f_n^{-1} : B_{r_n}(0) \ra B_{r_{n+1}}(0)$ die Bedingungen $h(0)=0$
  und $h'(0) = 1$ erfüllt, also $1 = |h'(0)| \leq \frac{r_{n+1}}{r_n}$ nach
  Proposition \ref{prop:kreis} gilt. 
  Wir setzen nun
  \[
  ...
  Abbildung \break$g: f(X) \ra B_r(0)$ mit $g(0) = 0$ und $g'(0) = 1$. 
  Sei nun $n$ so groß gewählt, dass $r_n > r$. Dann hätte die Funktion
  \[
  h:= g \circ f \circ f^{-1}_n : B_{r_n}(0) \ra B_r(0)
  \]
  die Eigenschaften $h(0)=0$ und $h'(0) = 1$, aber es wäre $r < r_n$. Dies ist ein Widerspruch zu
...
\begin{rem}
  ...
  dass die deRham"=Gruppe jeder einfach zusammenhängenden Riemannschen
  Fläche verschwindet und die Fläche dementsprechend
